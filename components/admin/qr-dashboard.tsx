...
function DetailDrawer({ code, detail, loading, onClose }: { code: string; detail: Detail | null; loading: boolean; onClose: () => void }) {
      ...
      <div className="h-full w-full max-w-xl overflow-y-auto bg-[#F5F8FB] shadow-2xl">
        ...
        {loading ? (
          ...
          <div className="space-y-5 p-6">
            ...
            <div className="rounded-2xl border border-[#EAECEF] bg-white p-4">
              <h4 className="mb-3 text-sm font-bold text-[#334155]">العملاء ({detail.items.length})</h4>
                      ...
                      {detail.items.map((r, i) => (
                        <tr key={i} className="border-b border-[#F5F7FA] last:border-none">
                          ...
                          <td className="px-2 py-2.5">
                            <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${r.status === "subscribed" ? "bg-emerald-50 text-emerald-700" : r.status === "contacted" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-600"}`}>
