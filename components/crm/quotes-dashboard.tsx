    ...
    <section className="mt-8 space-y-5">
      ...
      {formOpen && canEdit && (
        <form onSubmit={handleCreate} className="space-y-4 rounded-[28px] border border-[#EAECEF] bg-white p-6">
          ...
          <button type="submit" disabled={saving} className="rounded-xl bg-[#0F8B94] px-5 py-2 text-sm font-bold text-white">{saving ? "جاري الحفظ..." : "حفظ العرض"}</button>
        </form>
      )}
                  ...
                  <tr key={quote.id} className="border-b border-[#F1F5F9]">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-wrap gap-2">
                        <a href={`${QUOTES_API_URL}/${quote.id}?format=print`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg bg-[#F1FBFA] px-3 py-1 text-xs font-bold text-[#0F8B94]"><FileText className="h-3.5 w-3.5" />PDF</a>
